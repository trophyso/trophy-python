# This file was auto-generated by Fern from our API Definition.

from ..core.client_wrapper import SyncClientWrapper
import typing
from ..core.request_options import RequestOptions
from ..types.leaderboard_response import LeaderboardResponse
from ..core.pydantic_utilities import parse_obj_as
from ..errors.unauthorized_error import UnauthorizedError
from ..types.error_body import ErrorBody
from ..errors.unprocessable_entity_error import UnprocessableEntityError
from json.decoder import JSONDecodeError
from ..core.api_error import ApiError
from ..types.leaderboard_response_with_rankings import LeaderboardResponseWithRankings
from ..core.jsonable_encoder import jsonable_encoder
from ..errors.not_found_error import NotFoundError
from ..types.user_leaderboard_response import UserLeaderboardResponse
from ..core.client_wrapper import AsyncClientWrapper


class LeaderboardsClient:
    def __init__(self, *, client_wrapper: SyncClientWrapper):
        self._client_wrapper = client_wrapper

    def all_(
        self, *, request_options: typing.Optional[RequestOptions] = None
    ) -> typing.List[LeaderboardResponse]:
        """
        Get all active leaderboards for your organization.

        Parameters
        ----------
        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        typing.List[LeaderboardResponse]
            Successful operation

        Examples
        --------
        from trophy import TrophyApi

        client = TrophyApi(
            api_key="YOUR_API_KEY",
        )
        client.leaderboards.all_()
        """
        _response = self._client_wrapper.httpx_client.request(
            "leaderboards",
            method="GET",
            request_options=request_options,
        )
        try:
            if 200 <= _response.status_code < 300:
                return typing.cast(
                    typing.List[LeaderboardResponse],
                    parse_obj_as(
                        type_=typing.List[LeaderboardResponse],  # type: ignore
                        object_=_response.json(),
                    ),
                )
            if _response.status_code == 401:
                raise UnauthorizedError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 422:
                raise UnprocessableEntityError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            _response_json = _response.json()
        except JSONDecodeError:
            raise ApiError(status_code=_response.status_code, body=_response.text)
        raise ApiError(status_code=_response.status_code, body=_response_json)

    def get(
        self,
        key: str,
        *,
        offset: typing.Optional[int] = None,
        limit: typing.Optional[int] = None,
        run: typing.Optional[str] = None,
        user_id: typing.Optional[str] = None,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> LeaderboardResponseWithRankings:
        """
        Get a specific leaderboard by its key.

        Parameters
        ----------
        key : str
            Unique key of the leaderboard as set when created.

        offset : typing.Optional[int]
            Number of rankings to skip for pagination.

        limit : typing.Optional[int]
            Maximum number of rankings to return.

        run : typing.Optional[str]
            Specific run date in YYYY-MM-DD format. If not provided, returns the current run.

        user_id : typing.Optional[str]
            When provided, offset is relative to this user's position on the leaderboard. If the user is not found in the leaderboard, returns empty rankings array.

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        LeaderboardResponseWithRankings
            Successful operation

        Examples
        --------
        from trophy import TrophyApi

        client = TrophyApi(
            api_key="YOUR_API_KEY",
        )
        client.leaderboards.get(
            key="weekly-words",
            run="2025-01-15",
            user_id="user-123",
        )
        """
        _response = self._client_wrapper.httpx_client.request(
            f"leaderboards/{jsonable_encoder(key)}",
            method="GET",
            params={
                "offset": offset,
                "limit": limit,
                "run": run,
                "userId": user_id,
            },
            request_options=request_options,
        )
        try:
            if 200 <= _response.status_code < 300:
                return typing.cast(
                    LeaderboardResponseWithRankings,
                    parse_obj_as(
                        type_=LeaderboardResponseWithRankings,  # type: ignore
                        object_=_response.json(),
                    ),
                )
            if _response.status_code == 401:
                raise UnauthorizedError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 404:
                raise NotFoundError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 422:
                raise UnprocessableEntityError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            _response_json = _response.json()
        except JSONDecodeError:
            raise ApiError(status_code=_response.status_code, body=_response.text)
        raise ApiError(status_code=_response.status_code, body=_response_json)

    def users_leaderboards(
        self,
        user_id: str,
        key: str,
        *,
        run: typing.Optional[str] = None,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> UserLeaderboardResponse:
        """
        Get a user's rank, value, and history for a specific leaderboard.

        Parameters
        ----------
        user_id : str
            The user's ID in your database.

        key : str
            Unique key of the leaderboard as set when created.

        run : typing.Optional[str]
            Specific run date in YYYY-MM-DD format. If not provided, returns the current run.

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        UserLeaderboardResponse
            Successful operation

        Examples
        --------
        from trophy import TrophyApi

        client = TrophyApi(
            api_key="YOUR_API_KEY",
        )
        client.leaderboards.users_leaderboards(
            user_id="user-123",
            key="weekly-words",
            run="2025-01-15",
        )
        """
        _response = self._client_wrapper.httpx_client.request(
            f"users/{jsonable_encoder(user_id)}/leaderboards/{jsonable_encoder(key)}",
            method="GET",
            params={
                "run": run,
            },
            request_options=request_options,
        )
        try:
            if 200 <= _response.status_code < 300:
                return typing.cast(
                    UserLeaderboardResponse,
                    parse_obj_as(
                        type_=UserLeaderboardResponse,  # type: ignore
                        object_=_response.json(),
                    ),
                )
            if _response.status_code == 401:
                raise UnauthorizedError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 404:
                raise NotFoundError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 422:
                raise UnprocessableEntityError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            _response_json = _response.json()
        except JSONDecodeError:
            raise ApiError(status_code=_response.status_code, body=_response.text)
        raise ApiError(status_code=_response.status_code, body=_response_json)


class AsyncLeaderboardsClient:
    def __init__(self, *, client_wrapper: AsyncClientWrapper):
        self._client_wrapper = client_wrapper

    async def all_(
        self, *, request_options: typing.Optional[RequestOptions] = None
    ) -> typing.List[LeaderboardResponse]:
        """
        Get all active leaderboards for your organization.

        Parameters
        ----------
        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        typing.List[LeaderboardResponse]
            Successful operation

        Examples
        --------
        import asyncio

        from trophy import AsyncTrophyApi

        client = AsyncTrophyApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.leaderboards.all_()


        asyncio.run(main())
        """
        _response = await self._client_wrapper.httpx_client.request(
            "leaderboards",
            method="GET",
            request_options=request_options,
        )
        try:
            if 200 <= _response.status_code < 300:
                return typing.cast(
                    typing.List[LeaderboardResponse],
                    parse_obj_as(
                        type_=typing.List[LeaderboardResponse],  # type: ignore
                        object_=_response.json(),
                    ),
                )
            if _response.status_code == 401:
                raise UnauthorizedError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 422:
                raise UnprocessableEntityError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            _response_json = _response.json()
        except JSONDecodeError:
            raise ApiError(status_code=_response.status_code, body=_response.text)
        raise ApiError(status_code=_response.status_code, body=_response_json)

    async def get(
        self,
        key: str,
        *,
        offset: typing.Optional[int] = None,
        limit: typing.Optional[int] = None,
        run: typing.Optional[str] = None,
        user_id: typing.Optional[str] = None,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> LeaderboardResponseWithRankings:
        """
        Get a specific leaderboard by its key.

        Parameters
        ----------
        key : str
            Unique key of the leaderboard as set when created.

        offset : typing.Optional[int]
            Number of rankings to skip for pagination.

        limit : typing.Optional[int]
            Maximum number of rankings to return.

        run : typing.Optional[str]
            Specific run date in YYYY-MM-DD format. If not provided, returns the current run.

        user_id : typing.Optional[str]
            When provided, offset is relative to this user's position on the leaderboard. If the user is not found in the leaderboard, returns empty rankings array.

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        LeaderboardResponseWithRankings
            Successful operation

        Examples
        --------
        import asyncio

        from trophy import AsyncTrophyApi

        client = AsyncTrophyApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.leaderboards.get(
                key="weekly-words",
                run="2025-01-15",
                user_id="user-123",
            )


        asyncio.run(main())
        """
        _response = await self._client_wrapper.httpx_client.request(
            f"leaderboards/{jsonable_encoder(key)}",
            method="GET",
            params={
                "offset": offset,
                "limit": limit,
                "run": run,
                "userId": user_id,
            },
            request_options=request_options,
        )
        try:
            if 200 <= _response.status_code < 300:
                return typing.cast(
                    LeaderboardResponseWithRankings,
                    parse_obj_as(
                        type_=LeaderboardResponseWithRankings,  # type: ignore
                        object_=_response.json(),
                    ),
                )
            if _response.status_code == 401:
                raise UnauthorizedError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 404:
                raise NotFoundError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 422:
                raise UnprocessableEntityError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            _response_json = _response.json()
        except JSONDecodeError:
            raise ApiError(status_code=_response.status_code, body=_response.text)
        raise ApiError(status_code=_response.status_code, body=_response_json)

    async def users_leaderboards(
        self,
        user_id: str,
        key: str,
        *,
        run: typing.Optional[str] = None,
        request_options: typing.Optional[RequestOptions] = None,
    ) -> UserLeaderboardResponse:
        """
        Get a user's rank, value, and history for a specific leaderboard.

        Parameters
        ----------
        user_id : str
            The user's ID in your database.

        key : str
            Unique key of the leaderboard as set when created.

        run : typing.Optional[str]
            Specific run date in YYYY-MM-DD format. If not provided, returns the current run.

        request_options : typing.Optional[RequestOptions]
            Request-specific configuration.

        Returns
        -------
        UserLeaderboardResponse
            Successful operation

        Examples
        --------
        import asyncio

        from trophy import AsyncTrophyApi

        client = AsyncTrophyApi(
            api_key="YOUR_API_KEY",
        )


        async def main() -> None:
            await client.leaderboards.users_leaderboards(
                user_id="user-123",
                key="weekly-words",
                run="2025-01-15",
            )


        asyncio.run(main())
        """
        _response = await self._client_wrapper.httpx_client.request(
            f"users/{jsonable_encoder(user_id)}/leaderboards/{jsonable_encoder(key)}",
            method="GET",
            params={
                "run": run,
            },
            request_options=request_options,
        )
        try:
            if 200 <= _response.status_code < 300:
                return typing.cast(
                    UserLeaderboardResponse,
                    parse_obj_as(
                        type_=UserLeaderboardResponse,  # type: ignore
                        object_=_response.json(),
                    ),
                )
            if _response.status_code == 401:
                raise UnauthorizedError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 404:
                raise NotFoundError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            if _response.status_code == 422:
                raise UnprocessableEntityError(
                    typing.cast(
                        ErrorBody,
                        parse_obj_as(
                            type_=ErrorBody,  # type: ignore
                            object_=_response.json(),
                        ),
                    )
                )
            _response_json = _response.json()
        except JSONDecodeError:
            raise ApiError(status_code=_response.status_code, body=_response.text)
        raise ApiError(status_code=_response.status_code, body=_response_json)
